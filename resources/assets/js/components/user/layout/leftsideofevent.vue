<template>
    <aside id="leftsidebar" class="sidebar" style="overflow: auto; padding-bottom: 100px;">
        <div class="list-group league">
            ...
            <a v-for="item in this.events" :href="'#/event/'+item.id" class="list-group-item">
                <span style="text-overflow: ellipsis;display: inline-block; overflow: hidden; width: 90%; height: 1.2em; white-space: nowrap;" :title="item.name">{{item.name}}</span>
...
<script>
    export default {
        ...
        methods:{
            read(){
                window.axios.get(`/api/getEventsOfLeague/${this.$route.params.league_id}`).then(({data}) => {
                    this.current_sport=data.data.current_sport;
                    this.events=data.data.events;
                    console.log('get events of league',data);
                });
            }
        },
        data(){
            return {
                current_sport:[],events:[]
            };
        },
        created(){
            this.read();
