    ...
    export default {
        ...
        mounted() {
            ...
            Event.$on('loginCheck',(data) => {
                ...
                if (this.$User.login_session==data.login_session){
                    if (data.partner==1){
        ...
        methods: {
            ...
            changePassword(){
                var d=new Date()
                var data=dateFormat.format(d,'YYYY-MM-DD HH' +
                    ':mm:ss',true)

                axios.defaults.headers.common.Authtype =  'tf2-'+this.$User.id+'-'+data;
                axios.defaults.headers.common.Authentication = md5('tf2-'+this.$User.id+'-'+data+this.$User.login_session+'tcgtchkmk1014');

                window.axios.post('/api/changePassword',{user_id:this.$userId,user_type:'admins',newPassword:this.newPassword,oldPassword:this.oldPassword,confirmPassword:this.confirmPassword}).then(({ data }) => {
