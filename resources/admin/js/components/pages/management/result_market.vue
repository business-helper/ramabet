        ...
        <div class="card">
            ...
            <div class="body">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped table-hover dataTable " :class="'marketResultExport'+marketStatus">
                        <thead>
                        <tr>
                            <th class="">S.No. </th>
                            <th class="">Event</th>
                        ...
                        <tr v-for="item in markets" v-bind="item" v-if="c_sid==-1 || c_sid==item.sport_id">
                            ...
                            <td >
                                <button v-if="$isSuper==0 && item.marketStatus!=='OPEN'" type="button" class="btn light-green waves-effect" @click="Undeclare(item.id)">UnDeclare</button>
                            </td>
                        </tr>
                        </tbody>
                    </table>
...
<script>
    ...
    export default {
        data() {
            return {
                runners:[],is_show:'show',event:[],market:[],markets:[],currentMarketId:'',widthOfWindow:900,mdt:'',fancyMarkets:[],c_sid:-1
            }
        },
        props: [
        ...
        methods:{
            ...
            read1(){
                ...
                window.axios.post('/api/getMarketOfStatus', {marketStatus:this.marketStatus,userType:'admins',userId:this.$userId}).then(({data}) => {
                    this.markets=data.data.markets;
                    this.fancyMarkets=data.data.fancyMarkets;
                    console.log(this.marketStatus,data.data);
                    if (this.mdt!=="") this.mdt.destroy();
                    this.$nextTick(() => {
                        this.mdt = $('.marketResultExport'+this.marketStatus).DataTable({
                            dom: 'lBfrtip',
                            responsive: true,
                            buttons: [
                                'copy', 'csv', 'excel', 'pdf', 'print'
