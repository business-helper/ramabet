<div class="list-group">
    ...
    <a href="{{url('/'.$sports.'/'.$root)}}" class="list-group-item current_sports">
        <img class="{{$sports}}"/>
    ...
    @foreach($league as $item)
        <a href="{{url('/'.$sports.'/'.($root+1).'/'.json_decode($item)->id)}}" class="list-group-item" style="color: {{$b_color}}">
            <span style="text-overflow: ellipsis;display: inline-block;
    overflow: hidden;
    width: 90%;
    height: 1.2em;
    white-space: nowrap;" title="{{json_decode($item)->name}}">{{json_decode($item)->name}}</span>
