...
class PartnerLoginController extends Controller
{
    ...
    public function login(Request $request){
        ...
        if (Auth::guard('partner')->attempt(['email' => $request->email, 'password' => $request->password], $request->remember)) {
            ...
            $msg='Partner have login at '.$device .' in '.$user->last_login;
            $fireBase->SendNotification('loginCheck/admins/'.$id, ['link' => '/admin/logout', 'message' => $msg, 'userIds' => $id,'user_type'=>'admins','ip'=>$request->ip(),'device'=>$device,'login_session'=>$user->login_session,'partner'=>1]);
